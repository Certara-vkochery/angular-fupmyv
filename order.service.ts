...
export class OrdersService extends Subject<DataStateChangeEventArgs> {
  ...
  protected getData(
    ...
    let filtering = '';
    if (state.where) {
      let customerID = state.where[0].predicates[0].value;
      filtering = `$filter=CustomerID eq '${customerID}'&`;
    }
    ...
    return this.http
      .get(
        `${
          this.BASE_URL
        }?${filtering}${pageQuery}${sortQuery}&$inlinecount=allpages&$format=json`
      )
